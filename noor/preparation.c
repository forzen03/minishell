...
//STAGE 1 (EXECUTION PEREPARATION)
#include "../minishell.h"

// Check if command is builtin (echo, cd, pwd, export, unset, env, exit)
...
// Check if command is part of a pipeline (has pipe_in or pipe_out)
static int is_in_pipeline(t_cmd *c)
{
    return (c->pipe_in == 1 || c->pipe_out == 1);
}

// Determine execution type for a command (BUILTIN_PARENT, BUILTIN_CHILD, or EXTERNAL)
...
// Allocate and populate execution type array for all commands
...
// Main preparation function: allocate and initialize execution context from parsed commands
