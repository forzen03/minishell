...
int								check_pipe(char *line, int *i, t_token *node,
									t_token **tokens);
int								check_heredoc_and_append(char *line, int *i,
									t_token *node, t_token **tokens);
int								handle_operators(char *line, int *i,
									t_token **tokens);
char							*allocate_word_size(char *line, int *i,
									t_token **tokens);
int								is_quoted(char *line, int *i);
char							*allocate_word_size_double_quoted(char *line,
									int *i, t_token **tokens);
char							*allocate_word_size_single_quoted(char *line,
									int *i, t_token **tokens);
...
void							word_assign_double_quoted(t_token *node,
									char *word, int len);
int								check_quotes(char *line);
// t_token	*tokenizer(char *line);
// int		check_quotes(char *line);
int								return_value_of_quotes(int has_double,
									int has_single, int has_unquoted);
int								is_delimiter(char c);
void							skip_quote_section(char *line, int *i, int *len,
									char quote);
int								calculate_word_length(char *line, int *i);
void							extract_word_content(char *line, int *i,
									char *word);
void							init_for_quotes(int *has_double,
									int *has_single, int *has_unquotes);
t_token							*tokenizer(char *line);
...
void							ft_lstadd_back_redir(t_redir **lst,
									t_redir *new);
void							ft_lstclear_redirections(t_redir **lst,
									void (*del)(void *));

/* parser utilities */
t_token							*ft_lstlast_tokens(t_token *lst);
void							cmds_memory_allocation_failed(t_token *tokens,
									t_cmd *cmds);
...
t_cmd							*create_parser_node(t_token *tokens,
									t_cmd *cmds);

/* expander */
char							*ft_charjoin(char *s, char c);
void							join_ope(char *join, const char *s1,
									const char *s2);
void							memory_allocation_failed_expand(t_cmd *cmds,
									t_list *envc);
char							*expand_one_arg(char *s, t_list *envc,
									t_cmd *cmds);
char							*ft_strjoin_free(char *s1, char *s2);
char							*get_env_value(t_list *env, char *name,
									int *flag);
char							*expand_exit_status(char *res, int *i, char *s,
									t_cmd *cmds);
char							*expand_dollar(char *res, char *s, int *i,
									t_list *env);
void							expand_redirection(t_cmd *tmp, t_list *env,
									t_cmd *cmds);
void							expander(t_cmd *cmds, t_list *env);
int								get_new_len(t_cmd *cmd);
void							rebuild_argv(t_cmd *cmd, t_cmd *cmds,
									t_list *env);
void							rebuild_argv_loop(t_cmd *cmd, char **new_argv,
									int *new_quote_types);
void							free_two_strings(char *s1, char *s2);
void							expander_rebuild_argv_and_expand_redirections(
									t_cmd *tmp, t_cmd *cmds, t_list *env);
...
int								connect_pipes_for_child(int cmd_idx,
									t_execution *exec, t_cmd *cmd);
...
int								read_heredoc_content(t_redir *redir,
									t_list *env, t_cmd *cmd);
void							heredoc_sigint_handler(int sig);
void							heredoc_eof_warning(char *delimiter);
void							heredoc_cleanup_error(int *fd,
									struct sigaction *old_sa);
int								heredoc_fork_setup(t_heredoc_fork *setup);
void							heredoc_child(t_redir *redir, int write_fd,
									t_list *env, t_cmd *cmd);
void							close_heredoc_fds(t_cmd *cmd_list);
void							close_other_heredoc_fds(t_cmd *cmd_list,
									t_cmd *current_cmd);

/* execution - main */
void							execution(t_cmd *cmds, char **env,
									int *exit_status);
void							run_command_pipeline(t_execution *exec,
									t_list **env);

/* execution - child process */
int								child_status(int status);
void							execute_child_command(t_execution *exec,
									t_cmd *cmd, int i, t_list **env);
void							execute_external(t_cmd *cmd, t_list *env);

/* execution - builtins */
int								execute_builtin(t_cmd *cmd, t_list **env,
									int *exit_status);
int								execute_builtins_parent(t_execution *exec,
									t_list **env, int *exit_status);
void							update_env(t_list **env, char *key,
									char *value);
